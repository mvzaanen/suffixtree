...
template <class V>
suffixtree<V>::node::node(index b,
                       index e,
                       node *u,
                       suffixtree *o,
                       size_type num)
...
// recursively.
  for (typename map<typename suffixtree<V>::element_type,
      node *>::iterator i=children.begin(); i!=children.end(); i++) {
    delete i->second;
  }
}
...
suffixtree<V>::node::give_number() const {
...
template <class V>
void
suffixtree<V>::node::print_node(ostream &os, size_type depth) const {
...
// current node should be indented.
  ...
  for (typename map<typename suffixtree<V>::element_type, node *>::const_iterator i=children.begin();
...
template <class V>
typename suffixtree<V>::position
suffixtree<V>::node::SEA(index b,
                      index e,
                      size_type level,
                      node*& longest,
                      index cur,
...
template <class V>
vector<typename suffixtree<V>::element_type> 
suffixtree<V>::node::next_children() const {
// Collect all possible keys of this node and return them in a vector.
  typename map<typename suffixtree<V>::element_type, node *>::const_iterator i=children.begin();
...
template <class V>
const typename suffixtree<V>::node *
suffixtree<V>::node::find_child(const element_type &e) const {
// Find the child that was indexed with e. It returns 0 if the child
// doesn't exist.
  typename map<typename suffixtree<V>::element_type, node *>::const_iterator f_node=children.find(e);
...
template <class V>
void
suffixtree<V>::node::assign_child(const element_type e,
                               node *val) {
...
template <class V>
vector<typename suffixtree<V>::size_type>
suffixtree<V>::node::find_leaf_positions() const {
  vector<size_type> result;
  if (children.size()==0) {
    result.push_back(number);
  }
  for (typename map<typename suffixtree<V>::element_type, node *>::const_iterator c=children.begin();
      c!=children.end(); c++) {
...
template <class V>
const typename suffixtree<V>::size_type
suffixtree<V>::node::children_size() const {
  return children.size();
}

template <class V>
typename suffixtree<V>::node::node *
suffixtree<V>::node::find_sv(index cur,
                          index &b,
                          index e) {
...
template <class V>
typename suffixtree<V>::position
suffixtree<V>::node::seek_gamma(index &b,
                             index e) {
...
// seek_begin itself. It uses the skip count trick.
  typename suffixtree<V>::node *seek_node=this;
...
template <class V>
typename suffixtree<V>::position
suffixtree<V>::node::extend_gamma(index b,
                               index e,
                               index cur,
                               size_type level,
                               node *&longest,
...
// suffices need to be added.  (observation 1 and trick 2)
  ...
  if (cur==end()) {
    if (children.size()==0) { // rule 1
      ...
      typename map<typename suffixtree<V>::element_type, node *>::iterator place=children.find(new_el);
...
template <class V>
suffixtree<V>::suffixtree(value_type s)
...
template <class V>
typename suffixtree<V>::size_type
suffixtree<V>::find_position(value_type w) const {
// Check if sentence is a substring in the suffix tree. Throw a
// StringNotFound if it is not.
  typename suffixtree<V>::index i=w.begin(); // search index in w
  ...
  typename suffixtree<V>::index seeker=current->begin(); // begin of substring
  while (*i==*seeker) {
    i++;
    if (i==w.end()) {
      break;
...
template <class V>
vector<typename suffixtree<V>::size_type>
suffixtree<V>::find_all_positions(value_type w) const {
  typename suffixtree<V>::index i=w.begin(); // search index in w
...
template <class V>
bool
suffixtree<V>::self_test(value_type sentence) const {
...
template <class V>
void
suffixtree<V>::print(ostream &os) const {
...
template <class V>
void
suffixtree<V>::add_sentence(value_type s) {
// Build the suffix tree based on s.
  ...
  typename suffixtree<V>::index m=sentence.end();
  typename suffixtree<V>::index b=sentence.begin();
  for (typename suffixtree<V>::index i=b+2; i<=m; i++) { // i is `end' pointer
...
template <class V>
void
suffixtree<V>::SPA(typename suffixtree<V>::index i) {
...
// handling phase i (i+1 in Gusfields algorithm).
  ...
  for (typename suffixtree<V>::index j=sentence.begin()+j_index; j<i; j++) {
