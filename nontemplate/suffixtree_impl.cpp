...
suffixtree_impl::node::node(index b, index e, node* u,
...
const suffixtree_impl::index
suffixtree_impl::node::begin() const {
// Return the begin index, stored in begin_i.
   return begin_i;
}

const suffixtree_impl::size_type
suffixtree_impl::node::give_number() const {
// Return the number, stored in number.
   return number;
}

const suffixtree_impl::index
suffixtree_impl::node::end() const {
...
void
suffixtree_impl::node::print_node(ostream& os, size_type depth) const {
...
suffixtree_impl::position
suffixtree_impl::node::SEA(index b, index e, size_type level, node*&
...
void
suffixtree_impl::node::assign_child(const element_type e, node* val) {
// Set val as a child indexed with value e.
   children[e]=val;
}

vector<suffixtree_impl::size_type>
suffixtree_impl::node::find_leaf_positions() const {
...
suffixtree_impl::node::node*
suffixtree_impl::node::find_sv(index cur, index& b, index e) {
...
suffixtree_impl::position
suffixtree_impl::node::seek_gamma(index& b, index e) {
...
suffixtree_impl::position
suffixtree_impl::node::extend_gamma(index b, index e, index cur,
...
suffixtree_impl::suffixtree_impl(value_type s)
...
suffixtree_impl::size_type
suffixtree_impl::find_position(value_type w) const {
...
vector<suffixtree_impl::size_type>
suffixtree_impl::find_all_positions(value_type w) const {
   suffixtree_impl::index i=w.begin(); // search index in w
...
bool
suffixtree_impl::self_test(value_type sentence) const {
...
void
suffixtree_impl::print(ostream& os) const {
...
void
suffixtree_impl::add_sentence(value_type s) {
// Build the suffix tree based on s.
   ...
   suffixtree_impl::index b=sentence.begin();
   for (suffixtree_impl::index i=b+2; i<=m; i++) { // i is `end' pointer
      SPA(i); // do SPA based on Gusfields description of the algorithm
   }
}

void
suffixtree_impl::SPA(suffixtree_impl::index i) {
